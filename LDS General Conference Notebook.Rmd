...
How have the work length and the number of words per talk changed over the years? 
...
tidy_conference_all <- conference_talks %>%
  ...
  group_by(Date, Title) %>%
  #group_by(Title) %>%
  summarise(word_count = n(), average_word_length = mean(word_length)) %>%
  ungroup() %>%
  drop_na() %>%
  group_by(Date) %>%
  summarise(`Average Words/Talk` = mean(word_count),
            `Average Word Length` = mean(average_word_length)) %>%
  ungroup() %>%
  mutate(Date = as.numeric(str_replace(Date, "-", "."))) %>%
  gather(key = Type, value = Value, -Date)

ggplot(tidy_conference_all, aes(Date, Value)) +
  geom_point() +
